...
pub fn execute_vote(
    ...
    // update vote if vote previously cast
    BALLOTS.update(
        deps.storage,
        (proposal_id, &info.sender),
        |bal| -> StdResult<_> {
            if let Some(bal) = bal {
                prop.votes.remove_vote(bal.vote, vote_power);
            }
...
pub fn query(deps: Deps, env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::StakedAtPropHeight {
            proposal_id,
            address,
        } => to_json_binary(&query_staked_at_prop_height(
            deps,
            env,
            proposal_id,
            address,
...
fn query_staked_at_prop_height(
    deps: Deps,
    _env: Env,
    proposal_id: u64,
    address: Addr,
) -> StdResult<StakedAtPropHeightResponse> {
    let cfg = CONFIG.load(deps.storage)?;
    ...
    let vote_power = cfg
        .group_addr
        .is_voting_member(&deps.querier, &address, prop.start_height)?;

    let staked = vote_power.is_some();

    Ok(StakedAtPropHeightResponse {
        proposal_id,
        address,
        staked,
    })
